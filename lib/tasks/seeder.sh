...
tasks_seeder() {
  local dag_file="$TASKS_DIR/manifest/dag.json"
  if [ ! -s "$dag_file" ]; then
    port_log_error "❌ DAG not found. Run architect first."
    return 1
  fi
  ...
  while IFS= read -r task; do
    [ -n "$task" ] || continue
    local id deps_json deps_count target target_dir tmpfile deps_type
    ...
    mkdir -p "$target_dir" 2>/dev/null || { port_log_error "❌ Cannot create target directory $target_dir"; return 1; }

    tmpfile=$(mktemp "$target_dir/.tmp.${id}.XXXX") || { port_log_error "❌ Failed to create temp file for $target"; return 1; }
